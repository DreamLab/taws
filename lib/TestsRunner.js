 ...
 * @param {string} options.requestId test identifier which will be prepended to logs if present
 * @param {boolean} options.silentMode logs will be limited to minimum
 */
class TestsRunner {
    constructor(options = {}) {
        this.responseStack = [];
    ...
    /**
     * Tries to retireve error message from response object
    ...
    retrieveErrorMessage(response) {
        if (response.body) {
            try {
                ...
                this.printLog('Failed to retrieve error data', error);
            }
        }
        return {};
    }
    ...
    runTestsOnResponse(tests, response) {
        try {
            ...
            if (!statusCode || statusCode >= 400) {
                const errorData = this.retrieveErrorMessage(response);
                throw new Error(`Response error - ${statusCode}: ${JSON.stringify(errorData)}`);
            }
            const responseBody = typeof response.body === 'string' && response.headers['content-type'].includes('application/json') 
    ...
    fillTemplateWithData(data) {
        ...
        return map(data, (value) => {
            ...
            let newValue = !_.isEmpty(value) ? _.template(value)({$NOW: new Date().toISOString()}) : value;
            // run template on non empty values only
            return !_.isEmpty(newValue) ? _.template(newValue)({response: this.responseStack}) : newValue;
        });
    }
}

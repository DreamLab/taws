...
class TestsRunner {
    ...
    /**
     * Tries to retireve opal error message from response object
     * @param {Response} response express response object
     */
    retrieveOpalError(response) {
        if (response.body) {
            try {
                const responseBody = JSON.parse(response.body);
                return responseBody.err;
            } catch (error) {
                this.printLog('Failed to retrieve opal error data', error);
            }
        }
        return {};
    }
    ...
    runTestsOnResponse(tests, response) {
        try {
            ...
            if (!statusCode || statusCode >= 400) {
                const opalErrorData = this.retrieveOpalError(response);
                throw new Error(`Response error - ${statusCode}: ${JSON.stringify(opalErrorData)}`);
            }
